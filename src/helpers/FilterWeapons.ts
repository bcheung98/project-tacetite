export const filterWeapons = (weapons: any, filters: any, searchValue: string) => {
    let weps = [...weapons]
    if (filters.weaponType.length > 0) {
        weps = weps.filter(wep => filters.weaponType.includes(wep.type))
    }
    if (filters.rarity.length > 0) {
        weps = weps.filter(wep => filters.rarity.includes(wep.rarity.toString()))
    }
    ...
    if (filters.forgeryMat.length > 0) {
        weps = weps.filter(wep => filters.forgeryMat.includes(wep.materials.forgeryMat))
    }
