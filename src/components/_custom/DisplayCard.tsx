...
import { formatBossMats, formatCommonMats, formatForgeryMats, formatWeeklyBossMats } from "../../helpers/TooltipText"
...
function DisplayCard({
    ...
    const backgroundColor = () => {
        ...
        if (variant === "icon" || !showStars) {
            ...
            return `linear-gradient(${baseBG} ${gradient}, ${GetBackgroundColor(rarity, opacity)} 100%)`
        }
    }

    const handleHover = (direction: "enter" | "leave") => {
        !disableZoomOnHover && zoomImageOnHover(direction, `${id}-cardImage`, cardImageStyle.transform)
    }

    let imageURL = ""
    if (type === "character") { imageURL = `characters/${variant}s/${name}` }
    if (type === "weapon") { imageURL = `weapons/${name}` }
                        ...
                        >
                            {
                                ...
                                <Image src={`elements/icons/${info.element}`} alt={info.element} style={smallIconStyle} tooltip={{ title: info.element }} />
                            }
                            {
                                info.weapon !== undefined &&
                                <Image src={`weapons/icons/${info.weapon}`} alt={info.weapon} style={smallIconStyle} tooltip={{ title: info.weapon }} />
                            }
                        </Box>
                    }
                    <ButtonBase disableRipple href={href} target="_blank">
                        <Image src={imageURL} alt={name} id={`${id}-cardImage`} style={cardImageStyle} />
                    </ButtonBase>
                    <Box
                        sx={{
                            mt: variant === "icon" ? "0px" : type === "character" ? "-5px" : "60px",
                            borderBottom: variant === "icon" ? "none" : `calc(${size} / 30) solid ${GetRarityColor(rarity)}`,
                            position: "relative"
                        }}
                    >
                        <ButtonBase disableRipple href={href} target="_blank"
                ...
                >
                    {
                        ...
                        <Image src={`materials/forgery/${materials.forgeryMat}4`} alt={materials.forgeryMat} style={materialImageStyle} tooltip={{ title: formatForgeryMats(materials.forgeryMat) }} />
                    }
                    {
                        materials.bossMat &&
                        <Image src={`materials/boss/${materials.bossMat}`} alt={materials.forgeryMat} style={materialImageStyle} tooltip={{ title: formatBossMats(materials.bossMat) }} />
                    }
                    {
                        materials.weeklyBossMat &&
                        <Image src={`materials/weekly/${materials.weeklyBossMat}`} alt={materials.weeklyBossMat} style={materialImageStyle} tooltip={{ title: formatWeeklyBossMats(materials.weeklyBossMat) }} />
                    }
                    {
                        materials.commonMat &&
                        <Image src={`materials/common/${materials.commonMat}4`} alt={materials.commonMat} style={materialImageStyle} tooltip={{ title: formatCommonMats(materials.commonMat) }} />
                    }
                    {
                        materials.ascensionMat &&
                        <Image src={`materials/ascension/${materials.ascensionMat}`} alt={materials.ascensionMat} style={materialImageStyle} tooltip={{ title: materials.ascensionMat }} />
                    }
                </Box>
            }
        </Card>
