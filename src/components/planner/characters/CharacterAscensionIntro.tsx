...
function CharacterAscensionIntro({ character }: { character: CharacterCostObject }) {
    ...
    function Nodes() {
        return (
            <Box sx={{ display: "flex", alignItems: "center" }}>
                <Image
                    id={`${name}-intro-node1`}
                    src={`stat_icons/${forte.intro.nodes[0].type}`}
                    alt={forte.intro.nodes[0].type}
                    style={NodeIconStyle(selectedNode1)}
                    tooltip={{ title: parse(forte.intro.nodes[0].description) }}
                    onClick={handleSelectNode1}
                />
                <Xarrow start={`${name}-intro-node1`} end={`${name}-intro-node2`} showHead={false} path="grid" color="lightgray" strokeWidth={3} />
                <Image
                    id={`${name}-intro-node2`}
                    src={`stat_icons/${forte.intro.nodes[1].type}`}
                    alt={forte.intro.nodes[1].type}
                    style={NodeIconStyle(selectedNode2)}
                    tooltip={{ title: parse(forte.intro.nodes[1].description) }}
    ...
    React.useEffect(() => {
        dispatch(updateCharacterCosts({ name: name, type: "intro", subType: "main", costs: getCharacterForteCost(sliderValue, selectedMainNode) }))
        dispatch(updateCharacterCosts({ name: name, type: "intro", subType: "node1", costs: getCharacterForteCostNode("intro", 1, selectedNode1) }))
        dispatch(updateCharacterCosts({ name: name, type: "intro", subType: "node2", costs: getCharacterForteCostNode("intro", 2, selectedNode2) }))
    ...
    return (
        <Box sx={{ mb: "25px" }}>
            <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Image
                    id={`${name}-intro-main`}
                    src={`characters/skills/${name.toLowerCase()}_intro`}
                    alt={`${name.toLowerCase()}_intro`}
