...
function CharacterAscensionBasicATK({ character }: { character: CharacterCostObject }) {
    ...
    const { name, element, weapon, forte } = character
    ...
    function Nodes() {
        return (
            <Box sx={{ display: "flex", alignItems: "center" }}>
                <Image
                    id={`${name}-attack-node1`}
                    src={`stat_icons/${forte.attack.nodes[0].type}`}
                    alt={forte.attack.nodes[0].type}
                    style={NodeIconStyle(selectedNode1)}
                    tooltip={{ title: parse(forte.attack.nodes[0].description) }}
                    onClick={handleSelectNode1}
                />
                <Xarrow start={`${name}-attack-node1`} end={`${name}-attack-node2`} showHead={false} path="grid" color="lightgray" strokeWidth={3} />
                <Image
                    id={`${name}-attack-node2`}
                    src={`stat_icons/${forte.attack.nodes[1].type}`}
                    alt={forte.attack.nodes[1].type}
                    style={NodeIconStyle(selectedNode2)}
                    tooltip={{ title: parse(forte.attack.nodes[1].description) }}
    ...
    React.useEffect(() => {
        dispatch(updateCharacterCosts({ name: name, type: "attack", subType: "main", costs: getCharacterForteCost(sliderValue, selectedMainNode) }))
        dispatch(updateCharacterCosts({ name: name, type: "attack", subType: "node1", costs: getCharacterForteCostNode("attack", 1, selectedNode1) }))
        dispatch(updateCharacterCosts({ name: name, type: "attack", subType: "node2", costs: getCharacterForteCostNode("attack", 2, selectedNode2) }))
